...
public class ForgeNetworkHandler {
    ...
    public static final SimpleChannel SIMPLE_CHANNEL = NetworkRegistry.newSimpleChannel(
            new ResourceLocation(CavesAndCritters.MOD_ID, "network"),
            () -> PROTOCOL_VERSION,
    ...
    public static void init() {
        int idx = 0;
        for (Map.Entry<String, S2CPacket.Handler<?>> entry : S2CPacket.S2C_PACKETS.entrySet()) {
            registerMessage(idx++, entry.getValue());
        }
    }

    public static <T extends S2CPacket> void registerMessage(int idx, S2CPacket.Handler<T> handler) {
    ...
    public static <T extends S2CPacket> void handle(T packet, Supplier<NetworkEvent.Context> ctx, BiConsumer<T, Level> handle) {
    ...
    private static class Client {
        private static <T extends S2CPacket> void clientHandle(T packet, BiConsumer<T, Level> handle) {
